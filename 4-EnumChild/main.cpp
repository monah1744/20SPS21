...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpszCmdLine, 					int nCmdShow)
{
	...
	MessageBox(hWnd, 
	TEXT("Откройте, пожалуйста, \"Калькулятор\", и нажмите <CTRL>"), 	TEXT("Перечисление дочерних окон"), 
...
//у меня Калькулятор Плюс 
			if(!h)
				MessageBox(hWnd, 
				TEXT("Необходимо открыть \"Калькулятор\""), 						TEXT("Ошибка!!!"), MB_OK | MB_ICONSTOP);
			else
